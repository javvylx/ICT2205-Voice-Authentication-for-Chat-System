...
def enroll(name, file, speech):
    ...
    enroll_result = get_embedding(voice_model, file, p.MAX_SEC)
    #speech = speech_recognize(file)
    # enroll_embs = np.array(enroll_result.tolist())
    # speaker = name
    name = name.split("@")[0]
    file_name = name + ".npy"
    data = {'file_name': file_name, 'NPY_file': enroll_result.tolist(), 'Speech': speech}
    try:
...
def recognize(email, file):
    ...
    test_result = get_embedding(voice_model, file, p.MAX_SEC)
...
#enroll("JLBixby", os.path.abspath(r"C:\Users\chinb\Documents\GitHub\ict2205-crypto_pt2\voice_speech_authentication\audio\JLBixby.wav"))
#recognize("KK_test1", os.path.abspath(r"C:\Users\chinb\Documents\GitHub\ict2205-crypto_pt2\voice_speech_authentication\audio\KK_test1.wav"))