...
def enroll(name,file):
    ...
    try:
        if Path(p.MODEL_FILE+'/saved_model.pb').is_file():
            print("file exists")
...
if __name__ == '__main__':
    ...
    if get_extension(file)=='csv':
        if task == 'enroll':
            enroll_csv(file)
        if task == 'recognize':
            print("Recognize arguement cannot process a comma-seperated file. Please specify an audio file")
    else:
        if task == 'enroll':
            enroll(name, file)
        if task == 'recognize':
